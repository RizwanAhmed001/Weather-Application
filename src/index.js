...
function getForcastDays(name, days, index) {
    if(index === 0){
        ...
        return `<div id="currrentWeatherValues" class="flex flex-col w-full justify-center gap-3  p-2 items-center">
                    ...
                    <div id="cityDiv" class="bg-slate-800 flex justify-around items-center h-[20vh] rounded-2xl hover:bg-slate-600 w-full">
                        <h3 class="font-bold text-2xl">City:</h3>
                        <h3 id="city" class="text-3xl">${name}</h3>
                    </div>

                    <div id="dateDiv" class="bg-slate-800 w-full h-[20vh] flex items-center justify-around rounded-2xl hover:bg-slate-600">
                        <h3 class="font-bold text-2xl">Date:</h3>
                        <h3 id="date" class="text-3xl">${days.dt_txt.split(" ")[0]}</h3>
                    </div>

                    <div id="temperatureDiv" class="bg-slate-800 w-full h-[20vh] flex items-center justify-around rounded-2xl hover:bg-slate-600">
                    ...
                    <div id="weatherIconDiv" class="bg-slate-800 w-full h-[20vh] flex  items-center justify-around rounded-2xl hover:bg-slate-600 ">
                        <img class="h-[18vh] " id="currentweatherIcon" src="https://openweathermap.org/img/wn/${days.weather[0].icon}@4x.png" alt="Weather">
                        <h3 id="WeatherText" class="text-3xl">${days.weather[0].description}</h3>
                    </div>

                    <div id="humidityDiv" class="bg-slate-800 w-full h-[20vh] flex  items-center justify-around rounded-2xl hover:bg-slate-600">
                    ...
                    <div id="windDiv" class="bg-slate-800 w-full h-[20vh] flex items-center justify-around rounded-2xl hover:bg-slate-600">
                        <h3 class="font-bold text-2xl"><i class="fa-solid fa-wind text-sky-200 text-3xl"></i> Wind:</h3>
                        <h3 id="wind" class="text-3xl">${
                            days.wind.speed
                          }  M/S</h3>
                    </div>
                    <div id="sunriseDiv" class="bg-slate-800 w-full h-[20vh] flex items-center justify-around rounded-2xl hover:bg-slate-600">
                        <h3 class="font-bold text-2xl"><i class="fa-solid fa-wind text-3xl text-blue-700"></i> Gust:</h3>
                        <h3 id="sunrise" class="text-3xl">${days.wind.gust} M/S</h3>
                    </div>
                    <div id="sunsetDiv" class="bg-slate-800 w-full h-[20vh] flex items-center justify-around rounded-2xl hover:bg-slate-600">
                        <h3 class="font-bold text-2xl"><i class="fa-solid fa-volcano text-3xl text-red-300 "></i> Pressure:</h3>
                        <h3 id="sunset" class="text-3xl">${days.main.pressure} hPa</h3>
                    </div>
        </div>`
    
    }
    else{
        return `<div id="day1" class="p-3 flex flex-col mt-[5px] mb-[5px] bg-slate-800  rounded-2xl w-full hover:bg-slate-600">

                        <h3 id="fdate1" class="text-[1.25rem]">${
                          days.dt_txt.split(" ")[0]
                        }</h3>

                        <img src="https://openweathermap.org/img/wn/${days.weather[0].icon
  ...
  fetch(
    ...
    .then((data) => {
      ...
      forecast.forEach((days,index) => {
        if(index === 0){
            todayWeatherDiv.insertAdjacentHTML("beforeend", getForcastDays(name, days, index));
        }
        else{
            forcastDiv.insertAdjacentHTML("beforeend", getForcastDays(name, days, index));
        }
      });
    })
    .catch(() => {
      alert("Some error occur while fetching data!");
    });
}

function getUserLocation() {
  let uservalue = userInput.value.trim();
  if (!uservalue) return alert("Empty Search");
